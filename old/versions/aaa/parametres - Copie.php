...
function chargerUtilisateurs(idDossier) {
  ...
  // Vérification de l'ID dossier
  if (!idDossier || isNaN(idDossier)) {
    utilisateursContent.innerHTML = "<p class='text-red-500'>ID dossier invalide.</p>";
    return;
  }

  utilisateursContent.innerHTML = "<p class='text-gray-400'>Chargement des utilisateurs...</p>";
  ...
  .then(data => {
    if (data.status === 'success') {
      utilisateursContent.innerHTML = `
        <div>
          ...
          <div class="flex items-center justify-between mb-4">
            <button class="bg-blue-600 text-white px-4 py-2 rounded hover:bg-blue-700" onclick="ouvrirModalAjoutUtilisateur(${idDossier})">➕ Ajouter un utilisateur</button>
          </div>
      ...
      bindActionsUtilisateurs(idDossier);
    } else {
...
function initUtilisateurEvents() {
  // Bouton d'ajout
